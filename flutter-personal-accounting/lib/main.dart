            ...
            child: _records.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final catName = (record['category'] ?? '未分类') as String;
                      final cat = CategoryConfig.getCategory(isExpense ? 0 : 1, catName);
                      ...
                      return Dismissible(
                        ...
                        onDismissed: (_) async {
                          await DatabaseHelper.instance.deleteRecord(record['id'] as int);
                          _loadData();
                        ...
                        child: Card(
                          ...
                          child: ListTile(
                            ...
                            subtitle: Text(
                              '${record['date']}${record['note'] != null && (record['note'] as String).isNotEmpty ? ' · ${record['note']}' : ''}',
          ...
          Expanded(
            child: _stats.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      final catName = _stats.keys.elementAt(index);
                      final amount = _stats[catName]!;
                      final percent = _total > 0 ? amount / _total : 0.0;
                      final cat = CategoryConfig.getCategory(_type, catName);
          ...
          ListTile(
            ...
            onTap: () async {
              await Navigator.push(
                context,
                MaterialPageRoute(builder: (_) => const CategoryManagePage(type: 0)),
              );
              setState(() {});
            },
          ),
...
class AddRecordPage extends StatefulWidget {
  ...
  const AddRecordPage({Key? key, this.initialType, this.initialCategory}) : super(key: key);
...
class _AddRecordPageState extends State<AddRecordPage> {
  ...
  void initState() {
    ...
    _loadCategories().then((_) {
      if (widget.initialCategory != null) {
        ...
        setState(() => _category = _allCategories.first['name'] as String);
      }
    });
  }
  ...
  Future<void> _loadCategories() async {
    final db = await DatabaseHelper.instance.database;
    final list = await DatabaseHelper.instance.getCategories(_type);
    setState(() {
      _allCategories = list;
      if (list.isNotEmpty && _category.isEmpty) {
        _category = list.first['name'] as String;
      }
    });
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          if (categories.isEmpty)
            const Text('暂无分类，请先在设置中添加', style: TextStyle(color: Colors.grey)),
...
class _CategoryManagePageState extends State<CategoryManagePage> {
  ...
  Future<void> _load() async {
    final list = await DatabaseHelper.instance.getCategories(widget.type);
    setState(() => _categories = list);
  }

  String _getIcon(String? icon) => icon ?? '📦';
  Color _getColor(int? c) => c != null ? Color(c) : const Color(0xFF6C757D);
...
class CategoryConfig {
  ...
  static final defaultIncome = [
    ...
    {'name': '投资', 'icon': '📈', 'color': 0xFF011627, 'type': 1, 'is_system': 1},
...
class DatabaseHelper {
  ...
  Future<List<Map<String, dynamic>>> getCategories(int type) async {
    ...
    return db.query('categories',
        where: 'type = ?', whereArgs: [type], orderBy: 'is_system ASC, id ASC');
  }
